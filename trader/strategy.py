...
class StrategyConfig:
    ...
    def load(cls, path: Path) -> "StrategyConfig":
        try:
            ...
            config = cls(
                ...
                live_markets=[str(x).lower() for x in data.get("live_markets", [])],
                ...
                time_stop_loss_percent=float(data.get("time_stop_loss_percent", 0.3)),
                trailing_stop_activation_percent=float(data.get("trailing_stop_activation_percent", 1.5)),
    ...
    def validate(self) -> None:
        ...
        if any(x not in {"kr", "us"} for x in self.live_markets):
            raise StrategyError("live_markets에는 kr 또는 us만 지정할 수 있습니다.")
        ...
        if not (1 <= self.time_stop_minutes <= 240 and self.time_stop_loss_percent >= 0):
            raise StrategyError("시간 손절 설정을 확인하세요.")
        if not (0 < self.trailing_stop_giveback_percent < self.trailing_stop_activation_percent):
    ...
    def mode_for(self, market: str) -> str:
        if self.live_markets:
            return "live" if market in self.live_markets else "dry_run"
        return self.execution_mode

    @property
    def has_live_market(self) -> bool:
        return bool(self.live_markets) or self.execution_mode == "live"
